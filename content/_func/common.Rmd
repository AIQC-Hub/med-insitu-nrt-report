...
filer_locations_common <- function(df, min_lon, max_lon, min_lat, max_lat) {
  df_filtered <- df %>%
      group_by(platform_code, profile_no2) %>%
      ...
      distinct(platform_code, profile_no2)

  df %>% semi_join(df_filtered, by=c("platform_code", "profile_no2"))
}
...
get_unique_profile_no <- function(df) {
  df %>%
    distinct(platform_code, profile_timestamp, profile_no2, longitude, latitude) %>%
...
find_duplicates_within_platforms <- function(df) {
  df %>%
    ...
    arrange(platform_code, profile_timestamp, profile_no2, lon2, lat2) %>%
    group_by(platform_code, profile_timestamp, lon2, lat2) %>%
    summarise(profile_nos = paste0(profile_no2, collapse = ","),
              n = n(),
...
summarise_df_dup_within <- function(df_orig, df_dup) {
  df_orig  %>%
    select(platform_code, profile_timestamp, profile_no2, longitude, latitude,
...
find_duplicates_across_platforms <- function(df) {
  df %>%
    ...
    mutate(lon2 = round(longitude, 3),
           lat2 = round(latitude, 3)) %>%
    arrange(platform_code, profile_timestamp, profile_no2, longitude, latitude) %>%
    group_by(profile_timestamp, lon2, lat2) %>%
    summarise(platform_codes = paste0(platform_code, collapse = ","),
              profile_nos = paste0(profile_no2, collapse = ","),
              n = n(),
              n2 = length(unique(platform_code))) %>%
    filter((n > 1) & (n2 > 1)) %>%
...
summarise_df_dup_across <- function(df_orig, df_dup) {
  df_dup2 <- df_dup %>%
    mutate(
      ...
      profile_no2 = str_split(profile_nos, ",")
    ) %>%
    select(platform_code, profile_no2) %>%
    unnest(c(platform_code, profile_no2)) %>%
    mutate(profile_no2 = as.integer(profile_no2))
